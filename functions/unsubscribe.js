...
if((claims.uid == uid && claims.claims == "customer") || claims.admin === true){
  ...
  let date_now = package_id.substr(addit, key.length);
  ...
  let ref=`packagereg/${business_id}/${date_now}/subscription/${uid}`;
   ...
   dbref.set(null, function(error){
     if(error){
       res.status(500).json({error: 'An error occured'});
     }else{
       let ref2 = `registration/users/${uid}/subscription/${package_id}`;
       var dbref2 = db.ref(ref2);
       dbref2.set(null, function(error){
       if(error){
          res.status(500).json({error: 'An error occured'});
       }else{
         res.status(200).json({message: "Your have succesfully unsubscribed!!!"});
